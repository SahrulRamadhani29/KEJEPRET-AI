...
def load_image_from_url(url: str) -> np.ndarray:
    """Download foto dari URL (R2) dan konversi ke numpy array."""
    try:
...
def search_runner_photos(
    ...
    """
    Cari foto yang mengandung wajah pelari.
    Dipanggil saat pelari tekan 'Cari Foto Saya'.
    """
    cur = db.cursor()
    ...
    if not row:
        raise HTTPException(
            status_code=404,
            detail=f"Runner {request.runner_id} belum terdaftar. Silakan upload selfie."
        )
...
def delete_runner_embedding(
    runner_id: int,
    api_key: str = Depends(verify_api_key),
    db=Depends(get_db)
):
    """Hapus embedding pelari (saat akun dihapus)."""
    cur = db.cursor()
...
def delete_photo_embeddings(
    photo_id: int,
    api_key: str = Depends(verify_api_key),
    db=Depends(get_db)
):
    """Hapus embedding foto (saat foto dihapus FG/admin)."""
    cur = db.cursor()
